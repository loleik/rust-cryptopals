...
fn aes_ctr(input: &Vec<u8>, key: &str, nonce: u64) -> Vec<u8> {
    ...
    for i in 0..input_blocks.len() {
        ...
        let intermediate: Vec<u8> = aes_block_encrypt(&ctr_block, key.as_bytes());
...
pub fn challenge_18(input: &str) {
    ...
    let output: Vec<u8> = aes_ctr(&inp, "YELLOW SUBMARINE", 0);
